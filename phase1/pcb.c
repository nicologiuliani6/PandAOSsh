...
void freePcb(pcb_t* p) {
    if (p != NULL) list_add_tail(&p->p_list, &pcbFree_h); // reinserisce in coda alla lista dei liberi
}
...
pcb_t* allocPcb() {
    ...
    pos = pcbFree_h.next;
    list_del(pos);
    new_pcb = container_of(pos, pcb_t, p_list); // ottiene il PCB corrispondente
...
void insertProcQ(struct list_head* head, pcb_t* p) {
    struct list_head* pos;
    pcb_t* iter;
    // inserimento ordinato per priorità
    ...
    list_add_tail(&p->p_list, head); // se non ha priorità maggiore, va in coda
}
